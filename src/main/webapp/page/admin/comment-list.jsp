...
<script type="text/javascript">
                ...
                if (commentList.length > 0) {
                    ...
                    for (let i = 0; i < commentList.length; i++) {
                        ...
                        str += '<tr>' +
                            ...
                            '<td>' + updateTime + '</td>' +
                            '<td class="td-manage">' +
                            '<button type="button" class="layui-btn layui-btn-sm layui-btn-normal" onclick="reply(' + commentList[i].id + ')">回复</button>' +
                            '</td>' +
                            '</tr>';
                    }
                    $("#tab").html(str);
                } else {
                    $("#tab").html('<tr><td colspan="7" align="center">没有留言数据</td></tr>');
                }
            },
            error: function () {
                layer.msg("访问留言接口失败");
    ...
    //打开留言详情
    function reply(id) {
        let url = '../admin/comment/getDetailInfo?id=' + id;
        layer.open({
            type: 2,
            title: '回复留言',
            area: ['500px', '450px'],
